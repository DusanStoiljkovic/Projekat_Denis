...
def prve_kolo(n,area):            
    ...
    while True:     # toto dava panacika "A" do startnej pozicie ked spadne number6
        ...
        if number == 6:
            ...
            return board, current_position_row, current_position_column
...
# pohyb panacika A
def pohyb_hraca_1(area, n, position_row, position_column):            # tu mi nastaje problem jer neznam da napravim algoritam
    board = area
    players_per_team = round((n-3)/2)
    x = players_per_team
    forward_position_row = position_row
    forward_position_column = position_column
    print("row", forward_position_row,"column",forward_position_column)
    ...
    if forward_position_column > n//2:                  ## TOTO DOLE JE PEKLO 
        if forward_position_column == (n//2)+1:
            if number<= players_per_team:
                if forward_position_row == players_per_team:
                    board[players_per_team][forward_position_column+abs(number- players_per_team)] = "A"
                    board[forward_position_row][forward_position_column] = "*"
                    current_position_row = players_per_team
                    ...
                    if forward_position_row == players_per_team:
                        ...
                        current_position_column = forward_position_column+(number- players_per_team)
                        board[forward_position_row+number][forward_position_column] = "A"
                        board[forward_position_row][forward_position_column] = "*"
##                        current_position_row = forward_position_row+cislo
...
##    if cislo+forward_position_row <= players_per_team:
##        board[forward_position_row+cislo][forward_position_column] = "A"
##        board[forward_position_row][forward_position_column] = "*"
##        current_position_row = forward_position_row+cislo
##        aktualna_pozicia_stlpcu = forward_position_column
##
##    elif cislo+forward_position_row >= players_per_team:
##        board[players_per_team][forward_position_row+2+cislo] = "A"
##        board[forward_position_row][forward_position_column] = "*"
##        current_position_row = players_per_team
##        aktualna_pozicia_stlpcu = forward_position_row+2+cislo
        
    
##    return board, current_position_row, aktualna_pozicia_stlpcu
...
def main_2(area,n,position_row,position_column):       # Tu se idigrava sve za  2deo projekta (isao sa na ideju da pamti krosle kordinate figure A)
    new_area, new_position_row, new_position_column = pohyb_hraca_1(area,n,position_row, position_column)
    draw_board(new_area)
    board = new_area
    position_row = new_position_row
    position_column = new_position_column
    print("Print z main2 :"+"rad",position_row,"stlpec",position_column)

    main_2(area,n, position_row, position_column)
...
draw_board(area)
board, position_row, position_column =  prve_kolo(n,area)
main_2(area,n,position_row, position_column)
